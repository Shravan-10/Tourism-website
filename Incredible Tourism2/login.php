    ...
    <h2>Login Portal</h2>
        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="POST">
            <div class="input-field">
                ...
                <input type="text" placeholder="Username" name="RName" id="">
            </div>
            <div class="input-field">
                <i class="fas fa-lock"></i>
                <input type="password" placeholder="Password" name="RPass" id="">
            </div>

            <button type="submit" name="Signin" onclick="JumpPage()">Sign In</button>
   ...
   if($_SERVER["REQUEST_METHOD"]=="POST"){
       if(isset($_POST['Signin'])){
             ...
             $Rpass = $_POST['RPass']; 
              ...
              $sql = "SELECT Uname,Upass FROM User_Register";
              $output = mysqli_query($conn,$sql);
                if(mysqli_num_rows($output)>0){
                   while($rows = mysqli_fetch_assoc($output)){
                       if($rows['Uname'] == $Rname && $rows['Upass'] == $Rpass){
                           
                           session_start();
                           $_SESSION['AdminId'] = $_POST['RName'];
                        ...
                        echo '<script>';
                        echo 'alert("Wrong Password or User name!")';
                        echo '</script>';
                       }
                   }
                }
